...
export function CaCrashRates({ data }: { data: CaCrashRateDataset }) {
  ...
  return <section className="mt-7"><p className="eyebrow">CPUC Mileage × NHTSA SGO Crashes</p><h2 className="text-xl font-semibold tracking-tight mt-2">California Reported Crashes per Mile</h2>
    ...
    <div className="grid sm:grid-cols-3 gap-3 mt-4">
      <div className="viz-card p-5"><span className="text-sm text-neutral-600">Reported crashes / million miles</span><strong className="block text-3xl text-[#123b69] mt-2">{(crashes / miles * 1e6).toFixed(2)}</strong><span className="text-sm text-neutral-500">{crashes.toLocaleString()} SGO incidents</span></div>
      <div className="viz-card p-5"><span className="text-sm text-neutral-600">Serious or fatal crashes / million miles</span><strong className="block text-3xl text-[#123b69] mt-2">{(severe / miles * 1e6).toFixed(3)}</strong><span className="text-sm text-neutral-500">{severe} crashes with serious or fatal injury alleged</span></div>
