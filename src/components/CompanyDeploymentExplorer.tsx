...
export function CompanyDeploymentExplorer({ locations, geometries, latestS2, vintages }: {
  ...
  return <div>
    ...
    <div className="grid lg:grid-cols-[240px_minmax(0,1fr)] gap-4 mt-4 items-start">
      ...
      </div>{company === "Waymo" && <label className="text-sm text-neutral-600">S2 release <select className="block mt-1 border border-neutral-300 bg-white rounded p-2 text-sm" value={vintage} onChange={e => setVintage(e.target.value)}>{vintages.vintages.slice().reverse().map(v => <option key={v.vintage_end} value={v.vintage_end}>{v.vintage_end.slice(0,4)}–{v.vintage_end.slice(4)}</option>)}</select></label>}</div>
