...
function OversightExplorer({ events }: { events: OversightItem[] }) {
  ...
  return <div className="mt-3">
    <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-2" aria-label="NHTSA oversight milestones">{sorted.map(event => <button key={event.id} type="button" onClick={() => setSelectedId(event.id)} aria-pressed={item.id === event.id} className={`w-full text-left rounded-lg border p-4 transition-colors ${item.id === event.id ? "bg-[#eaf2fa] border-[#123b69] ring-1 ring-[#123b69]" : "bg-white border-neutral-200 hover:border-[#80a9d0]"}`}><span className="block text-sm font-semibold text-[#0b1d33]">{event.title}</span><span className="block text-sm text-neutral-700 mt-2 leading-snug">{event.takeaway ?? event.summary.split(". ")[0]}</span><span className="block text-sm text-neutral-500 mt-3">{displayDate(event.date)} · {event.status.replaceAll("_", " ")}</span></button>)}</div>
    <article className="viz-card p-5 mt-3" aria-live="polite"><div className="eyebrow">NHTSA oversight · {displayDate(item.date)}</div><h3 className="text-xl font-semibold mt-2">{item.title}</h3><p className="text-base text-neutral-700 leading-relaxed mt-2">{item.summary}</p><a className="text-sm text-[#184f95] underline inline-block mt-3" href={item.source_url} target="_blank" rel="noreferrer">Read agency record ↗</a></article>
  </div>;
}
