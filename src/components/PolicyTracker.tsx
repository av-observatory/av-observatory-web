...
export function FederalTracker() {
  ...
  return <main className="max-w-6xl px-8 py-8">
    ...
    <div className="mt-6 text-xs text-neutral-500">Reviewed {data.as_of} · Data source: {source === "R2" ? "R2 live record" : "reviewed site snapshot"}.</div>
  </main>;
}
...
export function StateTracker() {
  ...
  return <main className="max-w-7xl px-5 sm:px-8 py-8">
    ...
    <h1 className="text-4xl font-semibold mt-3 text-[#0b1d33]">State Regulations</h1>
    <p className="mt-3 text-base text-neutral-600 max-w-3xl">Select a state to explore its AV operating framework, responsible agencies, regulations, decisions, and other current oversight mechanisms.</p>
    <div className="grid lg:grid-cols-[minmax(0,3fr)_minmax(0,2fr)] gap-4 items-start">
      ...
      <div className="min-w-0">
        <section className="viz-card p-6 sm:p-7" aria-live="polite">
          ...
          <p className="text-sm text-neutral-500 mt-6">Reviewed {state.verified_at} · {source === "R2" ? "R2 Live Record" : "Reviewed Site Snapshot"}.</p>
        </section>
      </div>
    </div>
  </main>;
