    ...
    <div className="viz-card p-4 min-w-0">
      ...
      <div className="max-h-[560px] overflow-y-auto divide-y divide-neutral-200" role="list" aria-label="FMVSS standards and rulemakings">
        {visible.map(rule => <div role="listitem" key={rule.id}><button type="button" onClick={() => setSelectedId(rule.id)} aria-pressed={item.id === rule.id} className={`text-left w-full px-3 py-3 rounded-md transition-colors ${item.id === rule.id ? "bg-[#eaf2fa] border-l-4 border-[#123b69]" : "hover:bg-neutral-50 border-l-4 border-transparent"}`}>
          ...
          <span className="block mt-1 text-sm text-neutral-600">{rule.status === "effective" ? "In effect" : "Agency review"}{"rulemaking_id" in rule ? " · 2022 ADS rule" : ""}</span>
        </button></div>)}
      </div>
    </div>
...
function OversightExplorer({ events }: { events: OversightItem[] }) {
  const sorted = [...events].sort((a, b) => (b.date ?? "").localeCompare(a.date ?? ""));
  const [selectedId, setSelectedId] = useState(sorted[0]?.id);
  const item = sorted.find(x => x.id === selectedId) ?? sorted[0];
  return <div className="grid md:grid-cols-[.8fr_1.2fr] gap-3 mt-3">
    <div className="viz-card p-3 space-y-1" aria-label="NHTSA oversight milestones">{sorted.map(event => <button key={event.id} type="button" onClick={() => setSelectedId(event.id)} aria-pressed={item.id === event.id} className={`w-full text-left p-3 rounded-md border-l-4 ${item.id === event.id ? "bg-[#eaf2fa] border-[#123b69]" : "border-transparent hover:bg-neutral-50"}`}><span className="block text-sm font-semibold text-[#0b1d33]">{event.title}</span><span className="block text-sm text-neutral-500 mt-1">{displayDate(event.date)} · {event.status.replaceAll("_", " ")}</span></button>)}</div>
    <article className="viz-card p-5" aria-live="polite"><div className="eyebrow">NHTSA oversight · {displayDate(item.date)}</div><h3 className="text-xl font-semibold mt-3">{item.title}</h3><p className="text-sm text-neutral-700 leading-relaxed mt-3">{item.summary}</p><a className="text-sm text-[#184f95] underline inline-block mt-4" href={item.source_url} target="_blank" rel="noreferrer">Read agency record ↗</a></article>
  </div>;
}
...
function StateHistoryExplorer({ history, stateCode }: { history: PolicyEvent[]; stateCode: string }) {
  ...
  const groups = stateCode === "CA" ? ["All", "DMV", "CPUC", "State law"] : ["All"];
  ...
  return <div className="mt-3">
    ...
    <div className="max-h-64 overflow-y-auto border-y border-neutral-200 divide-y divide-neutral-200" aria-label="Select a policy event">{visible.map(entry => <button key={entry.id} type="button" onClick={() => setSelectedId(entry.id)} aria-pressed={item.id === entry.id} className={`w-full text-left px-3 py-2.5 border-l-4 ${item.id === entry.id ? "bg-[#eaf2fa] border-[#123b69]" : "border-transparent hover:bg-neutral-50"}`}><span className="block font-medium text-sm text-[#0b1d33]">{entry.title}</span><span className="block text-sm text-neutral-500 mt-0.5">{displayDate(entry.date)} · {entry.status.replaceAll("_", " ")}</span></button>)}</div>
    <article className="rounded-lg bg-[#f4f8fc] border border-[#d3dfed] p-4 mt-3" aria-live="polite"><div className="text-sm font-medium text-[#184f95]">{displayDate(item.date)} · {item.instrument.replaceAll("_", " ")}</div><h4 className="font-semibold mt-2">{item.title}</h4><p className="text-sm text-neutral-700 mt-2 leading-relaxed">{item.summary}</p><a href={item.source_url} target="_blank" rel="noreferrer" className="text-sm text-[#184f95] underline inline-block mt-3">{item.source_label ?? "Source text"} ↗</a>{item.source_tier === "secondary" && <p className="text-sm text-amber-800 mt-2">Secondary survey; primary legislation link pending review.</p>}</article>
  </div>;
}
...
export function FederalTracker() {
  ...
  return <main className="max-w-6xl px-8 py-8">
    ...
    <section className="mt-7"><h2 className="text-xl font-semibold">NHTSA oversight history</h2><p className="text-sm text-neutral-600 mt-1">Select a milestone to inspect the agency action and its source.</p><OversightExplorer events={data.federal_oversight} /></section>
