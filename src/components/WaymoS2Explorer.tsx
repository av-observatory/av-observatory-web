...
export function WaymoS2Explorer({
  ...
  const vintage = summary.latest_vintage;
  ...
  const vintageFeatures = geojson.features;
  ...
  return <div>
    ...
    <div className="grid lg:grid-cols-[1.55fr_.45fr] gap-3 mt-3">
      <div className="viz-card p-4">
        <div className="flex items-baseline justify-between gap-3">
          <div>
            ...
            <div className="text-sm text-neutral-500">Latest published S2 release ({vintageLabel(vintage)}). Waymo operational mileage is deployment exposure, not a testing ODD.</div>
          </div>
          <span className="text-sm text-neutral-500">{features.length.toLocaleString()} cells</span>
        </div>
      ...
      <div className="viz-card p-4">
        ...
        <div className="mt-2 text-xs text-neutral-500">Earlier releases before 2025-12 do not contain the newer explicit state/county fields, but their S2 geometry is retained.</div>
      </div>
    </div>
  </div>;
}
