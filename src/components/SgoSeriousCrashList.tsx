...
export function SgoSeriousCrashList({ crashes, media, reviewedSources }: { crashes: SeriousCrash[]; media: MediaContext[]; reviewedSources: ReviewedSource[] }) {
  return <section className="mt-7"><div className="flex flex-wrap justify-between items-end gap-3"><div><p className="eyebrow">NHTSA SGO · Latest Report Versions</p><h2 className="text-xl font-semibold tracking-tight mt-2">Serious or Fatal Crashes</h2></div><span className="text-sm text-neutral-600">{crashes.length} distinct reported incidents</span></div>
    <p className="text-sm text-neutral-600 mt-2 max-w-4xl">A running list of ADS reports with serious injury or fatality alleged. Reports sharing NHTSA’s Same Incident ID appear once, with all reporting entities and IDs shown. Severity is reported or alleged, and does not establish fault or final injury outcome. Source matches may be confirmed by report ID or only probable from date, city and collision details; the distinction appears on each entry. The latest NHTSA release contains part of August 2026.</p>
    <div className="grid gap-3 mt-4">{crashes.map((crash, i) => <details key={crash.incident_key} className="viz-card px-5 py-4" open={i === 0}>
      ...
      {(() => { const item = media.find(m => crash.report_ids.split(';').map(x=>x.trim()).includes(m.report_id)); return item ? <div className="bg-[#f1f6fb] rounded p-4 mt-4"><strong className="text-sm text-[#123b69]">{item.match.includes('agency_context_only') ? 'Agency Context' : 'Narrative Developed From Media and Agency Reports'}</strong><p className="text-sm leading-relaxed mt-2">{item.narrative}</p><p className="text-sm text-neutral-600 mt-2">{item.attribution} · <strong>{item.match.startsWith('exact_') ? 'Matched by NHTSA Report ID' : item.match.startsWith('probable_') ? 'Probable Match; Report ID Not Cited by Media' : 'Matched by Event Details; Report ID Not Cited by Media'}</strong></p><div className="flex flex-wrap gap-3 mt-2">{item.sources.map((url,j)=><a key={url} href={url} className="text-sm text-[#184f95] underline">Source {j+1} ↗</a>)}</div></div> : <p className="text-sm text-neutral-600 mt-4">No independently matched media account has been verified for this report. The details above come from the SGO fields and should not be read as a complete account of the crash.</p>; })()}
