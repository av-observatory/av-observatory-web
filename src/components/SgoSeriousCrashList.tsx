...
export function SgoSeriousCrashList({ crashes }: { crashes: SeriousCrash[] }) {
  return <section className="mt-7"><div className="flex flex-wrap justify-between items-end gap-3"><div><p className="eyebrow">NHTSA SGO · Latest Report Versions</p><h2 className="text-xl font-semibold tracking-tight mt-2">Serious or Fatal Crashes</h2></div><span className="text-sm text-neutral-600">{crashes.length} distinct reported incidents</span></div>
    <p className="text-sm text-neutral-600 mt-2 max-w-4xl">A running list of ADS reports with serious injury or fatality alleged. Reports sharing NHTSA’s Same Incident ID appear once, with all reporting entities and IDs shown. Severity is reported or alleged, and does not establish fault or final injury outcome. The latest NHTSA release contains part of August 2026.</p>
