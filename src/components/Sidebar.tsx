...
export function Sidebar() {
  ...
  return <aside className="w-60 shrink-0 bg-[#0b1d33] text-white flex flex-col min-h-screen">
    ...
    <nav className="flex-1 px-3 pb-5 space-y-5" aria-label="Main navigation">
      ...
      {GROUPS.map(group => <section key={group.title}><h2 className="px-3 mb-1 text-[13px] font-bold uppercase tracking-[.12em] text-[#8bb1d5]">{group.title}</h2><div className="space-y-0.5">{group.items.map(([href,label]) => <Link key={href} href={href} aria-current={path === href ? "page" : undefined} className={`block rounded-md px-3 py-2 text-[14px] leading-snug transition-colors ${path === href ? "bg-[#3987e5]/25 text-white font-semibold border-l-[3px] border-[#6fbbe9]" : "text-white/70 hover:text-white hover:bg-white/10"}`}>{label}</Link>)}</div></section>)}
    </nav>
